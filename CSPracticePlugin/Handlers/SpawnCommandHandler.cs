...
public class SpawnCommandHandler : ISpawnCommandHandler
{
	...
	public void Handle(CCSPlayerController? player, string text, ILogger logger)
	{
		...
		if (vector == null)
		{
			...
			player?.PrintToChat($"T: {TSpawnCoordinates.Keys.First()}-{TSpawnCoordinates.Keys.Last()}");
			player?.PrintToChat($"CT: {CTSpawnCoordinates.Keys.First()}-{CTSpawnCoordinates.Keys.Last()}");
			return;
		}
